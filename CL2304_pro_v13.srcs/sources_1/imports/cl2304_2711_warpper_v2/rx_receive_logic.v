module rx_receive_logic (
    ...
    output reg         [  15:0]         error_rd_cnt               ,
    output                              fifo_ds_rdclk              ,
    output reg                          fifo_ds_rdreq_r            ,
...
assign LED_R = (data_mod_i)? 'd0:
                    (error_rd_cnt != 'd0)?'d1:'d0;
...
always@(posedge sys_clk_i or negedge rst_n_i)begin
    if(!rst_n_i)
        ...
        if (state_cnt == 6 && fifo_ds_q == 16'd0) begin
            rd_m_cnt<='d8908;
        end
        else if (state_cnt == 6) begin
            rd_m_cnt<='d1044;
        end
        else
            rd_m_cnt<=rd_m_cnt;
...
always@(*)begin
    case (rd_state)
        RD_IDLE: fifo_ds_rdreq_o <= 'd0;
        RD_HEADER:
            if (fifo_ds_q == 16'hFDF7 && !fifo_ds_empty) begin
                fifo_ds_rdreq_o <= 'd1;
            end
            else
                fifo_ds_rdreq_o <= 'd0;
        default: fifo_ds_rdreq_o <= !fifo_ds_empty;
    endcase
end
...
always@(*)begin
    case (rd_state)
        RD_IDLE: fifo_ds_rdreq_r <= 'd0;
        default: fifo_ds_rdreq_r <= !fifo_ds_empty;
    endcase
end
...
always@(posedge sys_clk_i or negedge rst_n_i)begin
    if(!rst_n_i)
        error_rd_cnt<='d0;
    else if (veriy_cnt != fifo_ds_q && state_cnt == rd_m_cnt-1) begin
...
ila_debug u_ila_debug
    ...
    .probe0                            (frame_start_flag          ),// input wire [0:0]  probe0  
    ...
    .probe8                            (error_rd_cnt[7:0]         ),// input wire [7:0]  probe8 
    .probe9                            ({LOOP_EN,data_mod_i,fifo_ds_rdreq_o,data_v_MB[12:0]}),// input wire [15:0]  probe9 
    .probe10                           (rd_state                  ),// input wire [4:0]  probe10
    .probe11                           (state_cnt                 ),// input wire [15:0]  probe11
    .probe12                           (TX_Data                   ) // input wire [15:0]  probe12
    );
endmodule