...
// 
// File name:             pull_load_cc.v
// Create Date:           2025/01/07 16:21:24
// Version:               V1.0
// PATH:                  srcs\rtl\main_ctrl_pull_load\pull_load_cc.v
// Descriptions:          通过控制MOS管的Vgs来控制MOS管的通断电流,定电流拉载固定MOS管的通断电流
// 
// ********************************************************************************** // 
...
module pull_load_cc #(
    ...
    input  wire signed [AXI_REG_WIDTH-1: 0]target_i     ,//目标值mA
...
always@(posedge sys_clk_i)begin
    if (!rst_n_i) begin
        ...
        else if (short_state_add && (target_ctrl == target_i)) begin
            short_state_add <= 'd0;
        end
    end
    else begin
