...
always@(posedge sys_clk_i or negedge rst_n_i)begin
    if(!rst_n_i)
        ...
        if (rd_state == RD_TAIL) begin
            rd_m_cnt<='d1028;
        end
        else
        if (state_cnt == 2 && fifo_ds_q == 0) begin
            rd_m_cnt<='d8904;
        end
        else
            rd_m_cnt<=rd_m_cnt;
...
// assign fifo_ds_rdreq = !fifo_ds_empty && !fifo_prog_full_ddr3_us;
always@(posedge sys_clk_i) begin
    if((rd_state == RD_HEADER)|| (rd_state == RD_DATA)) begin
        fifo_ds_rdreq <= !fifo_ds_empty && !fifo_prog_full_ddr3_us;
    end
    else begin
        fifo_ds_rdreq <= 'b0;
    end
...
ila_debug u_ila_debug
    ...
    .probe4                            (fifo_ds_rdreq             ),// input wire [0:0]  probe4 
    ...
    .probe9                            ({LOOP_EN,data_v_MB[14:0]} ),// input wire [15:0]  probe9 
